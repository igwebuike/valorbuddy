...
function PartnerPortal({user,onLogout,previewId=null,onBack=null}){
  const[data,setData]=useState(null);const[message,setMessage]=useState('');const[busy,setBusy]=useState('');
  const plans=[
  ...
  async function load(){try{setData(await api(previewId?`/admin/partners/${previewId}/preview`:'/api/partner/overview'))}catch(e){setMessage(e.message)}}
  ...
  return <div className="partnerPortal"><header className="partnerHeader"><div className="partnerBrand"><Shield/><div><b>VALORBUDDY MARKETPLACE</b><span>{previewId?'Administrator portal preview':'A product of Tagus Technologies LLC'}</span></div></div><div className="partnerHeaderActions"><span className="veteranFreeBadge">Core access stays free for Veterans</span>{onBack&&<button onClick={onBack}>← Back to Admin</button>}<button onClick={onLogout}><LogOut/> Sign out</button></div></header><main className="partnerMain"><section className="partnerWelcome"><div><small>{previewId?'ADMINISTRATOR VIEW':'PARTNER COMMAND CENTER'}</small><h1>{org?.organization_name||'Your organization'}</h1><p>Connect trusted products, services, jobs, discounts and community resources with Veterans—while protecting their private conversations, documents and personal information.</p></div><div className="partnerStatusCard"><span className="partnerStatus">{String(org?.approval_status||'pending review').replaceAll('_',' ')}</span><b>{org?.plan_name||'Partner plan'}</b><small>{org?.monthly_price_cents?`$${(org.monthly_price_cents/100).toLocaleString()}/month`:'Custom pricing'}</small><em>Billing: {String(org?.billing_status||'not activated').replaceAll('_',' ')}</em></div></section>{message&&<div className="partnerMessage">{message}</div>}<section className="partnerMetrics">{Object.entries(data?.metrics||{}).map(([key,value])=><article key={key}><b>{Number(value).toLocaleString()}</b><span>{key.replaceAll('_',' ')}</span></article>)}</section><section className="partnerSection"><small>WHY VALORBUDDY</small><h2>One trusted front door to the Veteran economy</h2><div className="marketplaceValueGrid"><article><Car/><b>Everyday needs</b><span>Vehicles, fuel, housing, insurance, travel and home services.</span></article><article><Briefcase/><b>Opportunity</b><span>Jobs, training, entrepreneurship and veteran-owned suppliers.</span></article><article><Shield/><b>Trust</b><span>Verified partners, transparent offers and Veteran feedback.</span></article><article><BarChart3/><b>Measurable value</b><span>Privacy-safe referrals, redemptions and campaign outcomes.</span></article></div></section><section className="partnerSection"><small>COMMERCIAL PLANS</small><h2>{previewId?'Assign this organization’s marketplace plan':'Choose how your organization serves Veterans'}</h2><div className="partnerPlans">{plans.map(plan=><article className={org?.plan_code===plan.code?'selected':''} key={plan.code}><small>{plan.fit}</small><h3>{plan.name}</h3><b className="planPrice">{plan.price}</b><ul>{plan.features.map(x=><li key={x}>✓ {x}</li>)}</ul><button disabled={busy===plan.code||org?.plan_code===plan.code} onClick={()=>selectPlan(plan.code)}>{org?.plan_code===plan.code?'Current selection':busy===plan.code?'Saving…':previewId?'Assign plan':'Select plan'}</button></article>)}</div></section><section className="partnerBottomGrid"><article><small>PILOT ROADMAP</small><h2>Move from application to measurable impact</h2><ol>{(data?.next_steps||[]).map(x=><li key={x}>{x}</li>)}</ol></article><article><small>VETERAN TRUST</small><h2>Privacy-safe by design</h2><p>{data?.privacy_note}</p></article></section></main><footer className="partnerFooter">ValorBuddy is a product of Tagus Technologies LLC · Veterans first. Trust always.</footer></div>
}
