...
function Auth({onLogin}){
  ...
  async function submit(e){e.preventDefault();setErr('');try{const endpoint=tab==='partner'&&partnerMode==='apply'?'/auth/partner/register':tab==='register'?'/auth/register':'/auth/login';const d=await api(endpoint,{method:'POST',body:JSON.stringify(form)});onLogin(d.token,d.user)}catch(ex){setErr(ex.message)}}
