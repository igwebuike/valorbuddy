...
import{Shield,Mic,MicOff,LogOut,User,Calendar,MapPin,Heart,Folder,Music,Bell,Brain,Search,ChevronRight,BarChart3,Lock,Eye,EyeOff,Plus,Upload,FileText,RefreshCw,Radio,Menu,X,Trash2,Car,Home,Briefcase,Navigation,DollarSign,BookOpen,Dumbbell,Activity,Moon,Apple}from'lucide-react';
import'./style.css';
...
const voiceStyles={
  command:{label:'Command — strong, sharp & confident',rate:1.04,pitch:.92,volume:1},
  tactical:{label:'Tactical — crisp & authoritative',rate:1.09,pitch:.88,volume:1},
  clear:{label:'Clear — professional',rate:1.0,pitch:.98,volume:1},
...
function speak(text,{items=[],onDone}={}){
  ...
  const next=()=>{
    ...
    try{
      ...
      const chosen=available.find(v=>v.name===prefs.voiceName)||available.find(v=>/Microsoft.*(Guy|Mark|Christopher)|Google US English|Aaron|Daniel|Alex|David|Samantha/i.test(v.name))||available.find(v=>String(v.lang||'').toLowerCase().startsWith('en'));
...
function ValorBuddyApplication(){
  ...
  useEffect(()=>{if(!token){setLoading(false);return}api('/auth/me').then(u=>{const normalized=normalizeBranch(localStorage.getItem('valor_branch_theme')||u.branch);setUser({...u,branch:normalized});setBranchTheme(normalized)}).catch(()=>{localStorage.removeItem('valor_token');setToken(null);setUser(null)}).finally(()=>setLoading(false))},[token]);
  useEffect(()=>{if(!mobileNav)return;const previousOverflow=document.body.style.overflow;document.body.style.overflow='hidden';const closeOnEscape=e=>{if(e.key==='Escape')setMobileNav(false)};const closeOnDesktop=()=>{if(window.innerWidth>900)setMobileNav(false)};window.addEventListener('keydown',closeOnEscape);window.addEventListener('resize',closeOnDesktop);return()=>{document.body.style.overflow=previousOverflow;window.removeEventListener('keydown',closeOnEscape);window.removeEventListener('resize',closeOnDesktop)}},[mobileNav]);
  if(loading)return <div className="boot"><Shield/>ValorBuddy loading...</div>;
  if(!token||!user)return <Auth onLogin={(t,u)=>{const normalized=normalizeBranch(u.branch);localStorage.setItem('valor_token',t);applyBranchTheme(normalized);setBranchTheme(normalized);setToken(t);setUser({...u,branch:normalized})}}/>;
  const logout=()=>{localStorage.removeItem('valor_token');setToken(null);setUser(null);setMobileNav(false)};
...
function HealthSafetyNotice(){return <section className="healthSafetyNotice" aria-label="Important health and safety notice"><Shield/><div><strong>Important Health Notice</strong><p>ValorBuddy provides general informational and organizational support only. It is not a medical device and does not diagnose, treat, cure, or prevent any medical condition. Always consult a qualified healthcare professional for medical advice, diagnosis, medications, or treatment.</p></div><div className="healthSafetyActions"><a href="tel:911">Emergency: Call 911</a><a href="tel:988">Veterans Crisis Line: Call 988, then press 1</a></div></section>}
...
function Wellness({user,setScreen}){const[goal,setGoal]=useState('Build strength and stay mission-ready');const[days,setDays]=useState('3');const[details,setDetails]=useState('');const[plan,setPlan]=useState('');const[busy,setBusy]=useState(false);async function generate(){setBusy(true);setPlan('');const prompt=`Create a practical ${days}-day-per-week fitness and wellness plan for a ${user.service_status||'veteran'}. Goal: ${goal}. Preferences, equipment, or limitations: ${details||'none provided'}. Include strength or movement, mobility, cardio, sleep/recovery, and realistic nutrition habits. Keep it general wellness guidance, not medical diagnosis or treatment.`;try{const d=await api('/api/companion/chat',{method:'POST',body:JSON.stringify({message:prompt,timezone:Intl.DateTimeFormat().resolvedOptions().timeZone})});setPlan(cleanVisibleText(d.response||'Plan generated.'))}catch(e){setPlan(`ValorBuddy could not generate the plan right now: ${e.message}`)}finally{setBusy(false)}}return <section className="page wellnessPage"><div className="pageTitleRow"><div><h1>Fitness & Wellness</h1><p className="pageLead">Build practical routines for strength, mobility, cardio, sleep, recovery, and everyday nutrition. Adapt the plan to your current ability and stop if something does not feel right.</p></div><Dumbbell/></div><div className="wellnessPillars"><article><Dumbbell/><h3>Strength</h3><p>Simple progressive work for everyday capability.</p></article><article><Activity/><h3>Mobility & Cardio</h3><p>Movement, walking, conditioning, and flexibility.</p></article><article><Moon/><h3>Sleep & Recovery</h3><p>Recovery habits, pacing, hydration, and rest.</p></article><article><Apple/><h3>Nutrition Habits</h3><p>Practical meal and hydration habits without restrictive dieting.</p></article></div><div className="featureForm darkForm wellnessForm"><label><span>Primary wellness goal</span><select value={goal} onChange={e=>setGoal(e.target.value)}><option>Build strength and stay mission-ready</option><option>Improve mobility and flexibility</option><option>Improve cardio and endurance</option><option>Build a consistent healthy routine</option><option>Improve sleep and recovery habits</option><option>Manage weight with sustainable habits</option></select></label><label><span>Days per week</span><select value={days} onChange={e=>setDays(e.target.value)}><option value="2">2 days</option><option value="3">3 days</option><option value="4">4 days</option><option value="5">5 days</option></select></label><label className="wide"><span>Equipment, preferences, or limitations</span><textarea value={details} onChange={e=>setDetails(e.target.value)} placeholder="Example: home workouts, resistance bands, walking, low-impact movements..."/></label><button className="primary actionButton wideAction" type="button" disabled={busy} onClick={generate}><Dumbbell/>{busy?'Building your plan...':'Build my wellness plan'}</button></div>{plan&&<article className="wellnessPlan generatedDoc"><h2>Your ValorBuddy Wellness Plan</h2><p>{plan}</p><button type="button" onClick={()=>setScreen('companion')}><Brain/>Discuss this plan with ValorBuddy</button></article>}<p className="wellnessDisclaimer">General wellness information only. ValorBuddy is not a medical device and this feature does not diagnose or treat medical conditions. For medical concerns, exercise restrictions, or symptoms, consult a qualified healthcare professional.</p></section>}
...
function Admin(){const[o,setO]=useState(null);const[users,setUsers]=useState([]);const[busy,setBusy]=useState(null);const[msg,setMsg]=useState('');async function load(){const[overview,list]=await Promise.all([api('/admin/overview'),api('/admin/users')]);setO(overview);setUsers(list)}useEffect(()=>{load().catch(e=>setMsg(e.message))},[]);async function updateUser(u,changes){setBusy(u.id);setMsg('');try{await api(`/admin/users/${u.id}`,{method:'PATCH',body:JSON.stringify(changes)});await load();setMsg(`${u.first_name||u.email} updated successfully.`)}catch(e){setMsg(e.message)}finally{setBusy(null)}}return <section className="page adminDashboard"><div className="adminTitleRow"><div><h1>Admin Dashboard</h1><p className="pageLead">Manage member access, activation status and ValorBuddy roles.</p></div><button type="button" className="adminRefresh" onClick={()=>load()}><RefreshCw/>Refresh</button></div>{msg&&<div className="adminMessage">{msg}</div>}{o&&<div className="stats">{Object.entries(o).map(([k,v])=><div key={k}><b>{v}</b><span>{k.replaceAll('_',' ')}</span></div>)}</div>}<div className="adminUsersHeading"><h2>Users</h2><span>{users.length} accounts</span></div><div className="adminUserGrid">{users.map(u=><article className="adminUserCard" key={u.id}><div className="adminUserTop"><div className="adminAvatar">{(u.first_name||u.email||'?').charAt(0).toUpperCase()}</div><div className="adminIdentity"><h3>{[u.first_name,u.last_name].filter(Boolean).join(' ')||'ValorBuddy member'}</h3><a className="adminEmail" href={`mailto:${u.email}`} title={u.email}>{u.email}</a></div><span className={`adminStatus ${u.active?'active':'inactive'}`}>{u.active?'Active':'Inactive'}</span></div><div className="adminMeta"><span><b>Role</b>{u.role||'veteran'}</span><span><b>Service</b>{u.branch||'—'}</span><span><b>Location</b>{[u.city,u.state].filter(Boolean).join(', ')||'—'}</span></div><div className="adminControls"><label><span>Access role</span><select value={(u.role||'veteran').toLowerCase()} disabled={busy===u.id} onChange={e=>updateUser(u,{role:e.target.value})}><option value="veteran">Veteran / Member</option><option value="member">Member</option><option value="partner">Partner</option><option value="admin">Admin</option></select></label><button type="button" disabled={busy===u.id} className={u.active?'adminDeactivate':'adminActivate'} onClick={()=>updateUser(u,{active:!u.active})}>{busy===u.id?'Updating…':u.active?'Deactivate':'Approve / Activate'}</button></div></article>)}</div></section>}
