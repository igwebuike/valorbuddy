...
const voiceStyles={
  command:{label:'Command — strong, sharp & confident',rate:1.02,pitch:.92,volume:1},
  tactical:{label:'Tactical — crisp & authoritative',rate:1.08,pitch:.88,volume:1},
...
function Wellness({user,setScreen}){const[goal,setGoal]=useState('general fitness');const[days,setDays]=useState('3');const[limitations,setLimitations]=useState('');const[plan,setPlan]=useState('');const[busy,setBusy]=useState(false);const presets=[['Strength','Build practical full-body strength with scalable exercises.',Dumbbell],['Mobility','Improve flexibility, joint mobility, balance, and everyday movement.',Activity],['Cardio','Build walking, running, cycling, or low-impact endurance gradually.',Heart],['Sleep & Recovery','Create a consistent sleep, hydration, recovery, and stress-management routine.',Moon],['Nutrition Habits','Build simple meal, hydration, and healthy eating habits without fad-diet claims.',Apple]];async function buildPlan(){setBusy(true);setPlan('');try{const prompt=`Create a practical ${days}-day-per-week ${goal} wellness plan for me. I am ${user.service_status||'a veteran'} from the ${user.branch}. My stated limitations/preferences are: ${limitations||'none provided'}. Keep it realistic, progressive, and easy to follow. Include warm-up, main activity, recovery, hydration, and a simple weekly goal. Do not diagnose conditions or prescribe treatment; tell me to stop and seek qualified care for concerning symptoms.`;const d=await api('/api/vapi/action',{method:'POST',body:JSON.stringify({message:prompt,query:prompt,first_name:user.first_name,email:user.email,branch:user.branch,city:user.city,state:user.state,user_type:user.user_type||user.service_status||'Veteran',...locationPayload(),timezone:Intl.DateTimeFormat().resolvedOptions().timeZone})});setPlan(cleanVisibleText(d.response||''))}catch(e){setPlan(e.message)}finally{setBusy(false)}}return <section className="page wellnessPage readablePage"><div className="pageTitleRow"><div><h1>Fitness & Wellness</h1><p className="pageLead">Build sustainable routines for strength, mobility, cardio, sleep, recovery, and everyday wellbeing. ValorBuddy adapts the plan to your goals without pretending to replace a clinician or trainer.</p></div><Dumbbell className="wellnessHeroIcon"/></div><div className="wellnessPresetGrid">{presets.map(([title,desc,Icon])=><button type="button" key={title} className={goal===title.toLowerCase()?'active':''} onClick={()=>setGoal(title.toLowerCase())}><Icon/><b>{title}</b><span>{desc}</span></button>)}</div><div className="featureForm darkForm wellnessBuilder"><label><span>Primary wellness goal</span><select value={goal} onChange={e=>setGoal(e.target.value)}><option value="general fitness">General fitness</option><option value="strength">Strength</option><option value="mobility">Mobility & balance</option><option value="cardio">Cardio & endurance</option><option value="sleep & recovery">Sleep & recovery</option><option value="nutrition habits">Nutrition habits</option></select></label><label><span>Days per week</span><select value={days} onChange={e=>setDays(e.target.value)}><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option></select></label><label className="wide"><span>Preferences, equipment, or limitations</span><textarea value={limitations} onChange={e=>setLimitations(e.target.value)} placeholder="Example: home workouts, walking, resistance bands, avoid high impact..."/></label><button type="button" className="primary actionButton wideAction" onClick={buildPlan} disabled={busy}><Brain/>{busy?'Building your plan...':'Build my wellness plan'}</button></div><div className="wellnessSafety"><Heart/><div><b>Health & safety</b><span>Start gradually. Stop activity for chest pain, severe shortness of breath, fainting, or other concerning symptoms and seek appropriate medical care. ValorBuddy provides general wellness information, not medical diagnosis or treatment.</span></div></div>{plan&&<article className="generatedDoc wellnessPlan"><div className="cardTop"><h2>Your ValorBuddy wellness plan</h2><button type="button" onClick={()=>speak(plan)}>🔊 Hear plan</button></div><p className="wellnessPlanText">{plan}</p><div className="cardActions"><button type="button" onClick={()=>setScreen('companion')}><Brain/>Ask ValorBuddy a follow-up</button></div></article>}</section>}
